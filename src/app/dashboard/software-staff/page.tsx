...
export default function SoftwareStaffPage() {
  const router = useRouter();
  const [statusFilter, setStatusFilter] = useState("all");
  const { tickets, loading, refresh } = useTickets({ status: statusFilter, type: "Software" });
  const [solution, setSolution] = useState("");
  const [resolvingId, setResolvingId] = useState<string | null>(null);
  const [selectedIds, setSelectedIds] = useState<string[]>([]);

  const handleBulkClose = async () => {
    if (!confirm(`Close ${selectedIds.length} tickets?`)) return;
    try {
      await Promise.all(
        selectedIds.map((id) =>
  ...
  if (loading)
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        <div className="w-8 h-8 border-[3px] border-slate-200 dark:border-slate-700 border-t-purple-600 rounded-full animate-spin" />
        <p className="text-sm text-slate-500 dark:text-slate-400">Loading software queue...</p>
      </div>
    );
    ...
    <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8 animate-fade-in pb-32">
      <div className="lg:col-span-3 space-y-8">
              ...
              {activeTickets.length > 0 && (
                <label className="flex items-center gap-1 text-sm text-slate-500 dark:text-slate-400 font-bold cursor-pointer">
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) setSelectedIds(activeTickets.map((t) => t.id));
                      else setSelectedIds([]);
                    }}
        ...
        {selectedIds.length > 0 && (
          <div className="flex items-center gap-3 p-4 bg-purple-600 text-white rounded-2xl mb-6 page-reveal">
            <span className="text-sm font-bold">{selectedIds.length} selected</span>
            <button
              onClick={handleBulkClose}
              className="text-xs font-bold px-3 py-1.5 bg-white/20 rounded-lg transition-colors"
            >
              Close all
            </button>
            <button
              onClick={() => setSelectedIds([])}
              className="text-xs text-white/60 ml-auto font-bold transition-colors"
            >
              Clear
            </button>
          </div>
        ...
        <div className="space-y-6">
          {activeTickets.length === 0 ? (
            ...
            activeTickets.map((ticket) => (
              <div
                key={ticket.id}
                className={`card p-6 group relative border-l-4 border-l-purple-500 ${
                  ticket.status === "in_progress"
                    ? "bg-amber-50 dark:bg-amber-900/20"
                    : "bg-white dark:bg-slate-800"
                } ${
                  ticket.priority === "urgent"
                    ? "border-2 border-red-500/50 shadow-[0_0_20px_rgba(239,68,68,0.15)]"
                    : ""
                }`}
              >
                  ...
                  <div className="space-y-4 flex-1">
                    <div className="flex items-center gap-3 flex-wrap">
                      <input
                        ...
                        className="w-4 h-4 rounded border-slate-300 text-purple-600 focus:ring-purple-500"
                      />
                      <span className="badge !px-3 !py-1 badge-purple">🐛 Software</span>
                      <span className={`badge !px-3 !py-1 ${ticket.status === "in_progress" ? "badge-amber" : "badge-slate"}`}>
                        {ticket.status.replace("_", " ")}
                      </span>
                      <SlaBadge ticket={ticket} />
                      <span className="text-sm text-slate-500 dark:text-slate-400 font-mono font-bold uppercase">
                        #{ticket.id.slice(0, 8)}
                      </span>
                    </div>
                    <div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm font-semibold text-slate-500 dark:text-slate-400 pt-2">
                      ...
                      {ticket.priority === "urgent" && (
                        <span className="text-red-500 font-extrabold flex items-center gap-1">
                          <span className="text-xl leading-none">!</span> URGENT
                        </span>
                      )}
                      {ticket.priority === "high" && (
                        <span className="text-amber-600 font-extrabold">HIGH PRIORITY</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex md:flex-col gap-3 min-w-[140px] mt-6 md:mt-0">
                    {ticket.status === "open" && (
                      <button
                        ...
                        className="btn-primary !bg-purple-600 w-full py-3 text-sm"
                      >
                        Start Working
                      </button>
                    )}
                ...
                {resolvingId === ticket.id && (
                  <form
                    onSubmit={(e) => handleResolve(e, ticket.id)}
                    className="mt-8 pt-8 border-t border-slate-200 dark:border-slate-700 space-y-4"
                  >
                    <div className="space-y-2">
                      <label className="text-sm font-bold tracking-tight">Resolution / Fix Applied</label>
                      <textarea
                        required
                        className="input-field min-h-[120px] text-lg"
                        placeholder="Describe the fix, commit reference, or workaround applied..."
                        value={solution}
                        onChange={(e) => setSolution(e.target.value)}
                      />
                    </div>
                    <div className="flex gap-3">
                      <button type="submit" className="btn-primary !bg-purple-600 py-3 px-8 text-sm">
                        Submit Resolution
                      </button>
                      <button type="button" onClick={() => setResolvingId(null)} className="btn-secondary py-3 px-8 text-sm">
                        Cancel
                      </button>
                    </div>
                  </form>
      ...
      <div className="space-y-8 mt-12 lg:mt-0">
        <div className="card p-8 h-fit bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-800/30">
          <h2 className="text-xl font-bold tracking-tight mb-6 pb-4 border-b border-purple-100 dark:border-purple-800/40">
            Recently Resolved
          </h2>
          <div className="space-y-4">
            {resolvedTickets.slice(0, 5).map((ticket) => (
              <div
                key={ticket.id}
                className="p-4 rounded-xl bg-white dark:bg-slate-900 cursor-pointer transition-colors shadow-sm"
                onClick={() => router.push(`/dashboard/ticket/${ticket.id}`)}
              >
        ...
        <div className="card p-6 bg-slate-50 dark:bg-slate-800/50">
          <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-3">Quick Stats</p>
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-sm text-slate-600 dark:text-slate-300">Open</span>
              <span className="text-sm font-black text-slate-900 dark:text-white">{tickets.filter((t) => t.status === "open").length}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-slate-600 dark:text-slate-300">In Progress</span>
              <span className="text-sm font-black text-amber-600">{tickets.filter((t) => t.status === "in_progress").length}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-slate-600 dark:text-slate-300">Resolved</span>
              <span className="text-sm font-black text-emerald-600">{resolvedTickets.length}</span>
            </div>
          </div>
        </div>
      </div>
