    ...
    <div className="space-y-8 max-w-6xl pb-16 page-reveal bg-white dark:bg-slate-900 rounded-3xl p-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <h1 className="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">SLA Policies</h1>
          <p className="text-slate-400 dark:text-white/30 mt-1 text-sm font-medium">Manage Service Level Agreements</p>
        </div>
        <button
      ...
      <div className="bg-slate-100 dark:bg-slate-800/5 border border-slate-200 dark:border-white/10 rounded-2xl overflow-hidden">
        {policies.length === 0 ? (
          <div className="p-20 text-center">
            ...
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">No custom SLA policies</h3>
            <p className="text-slate-400 dark:text-white/40">Default SLA timers are active.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-800/3">
                  ...
                  <th className="p-4 text-xs font-bold text-slate-400 dark:text-white/40 uppercase tracking-widest">First Response</th>
                  <th className="p-4 text-xs font-bold text-slate-400 dark:text-white/40 uppercase tracking-widest">Resolution</th>
                  <th className="p-4 text-xs font-bold text-slate-400 dark:text-white/40 uppercase tracking-widest text-right">Actions</th>
                </tr>
              </thead>
              <tbody>
                {policies.map((policy) => (
        ...
        <div className="bg-slate-100 dark:bg-slate-800/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 mt-6 page-reveal">
          ...
          <form onSubmit={handleAddPolicy} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 dark:text-white/40 uppercase tracking-widest block mb-2">Ticket Type</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-400 dark:text-white/40 uppercase tracking-widest block mb-2">First Response</label>
                  <input
                    type="number"
                    required
                    className="w-full bg-slate-100 dark:bg-slate-800/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:outline-none focus:border-red-500/50 transition-colors"
                  ...
                  />
                  <p className="text-xs text-slate-400 dark:text-white/30 mt-1">60 = 1 hour</p>
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-400 dark:text-white/40 uppercase tracking-widest block mb-2">Resolution</label>
                  <input
                    type="number"
                    required
                    className="w-full bg-slate-100 dark:bg-slate-800/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:outline-none focus:border-red-500/50 transition-colors"
                  ...
                  />
                  <p className="text-xs text-slate-400 dark:text-white/30 mt-1">240 = 4 hours</p>
                </div>
              </div>
            </div>
