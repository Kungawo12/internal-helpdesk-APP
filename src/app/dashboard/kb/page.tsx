...
function KbPortal() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1e] rounded-2xl p-6 -m-4 md:-m-6 text-white">
      {selectedArticle ? (
        <div className="max-w-4xl mx-auto space-y-6 pb-16 page-reveal">
          ...
          <div className="bg-[#111827] border border-white/10 rounded-3xl p-8">
            ...
            <h1 className="text-3xl font-extrabold text-white tracking-tight mb-4">{selectedArticle.title}</h1>

            <div className="flex items-center gap-2 text-xs text-white/50 mb-8 border-b border-white/5 pb-4">
              <span className="font-bold text-white/70">{selectedArticle.author.name}</span>
              <span>·</span>
              <span>{new Date(selectedArticle.createdAt).toLocaleDateString()}</span>
            </div>

            <div className="prose prose-invert max-w-none">
              <pre className="whitespace-pre-wrap font-sans text-white/80 leading-relaxed text-base">
                {selectedArticle.content}
              </pre>
            </div>

            {selectedArticle.tags && (
              <div className="flex flex-wrap gap-1 mt-8 pt-4 border-t border-white/5">
                {selectedArticle.tags.split(",").map((tag) => (
                  <span key={tag} className="text-xs font-bold px-2 py-0.5 rounded-full bg-white/5 text-white/40 border border-white/10">
                    {tag.trim()}
                  </span>
                ))}
              </div>
        ...
        <div className="space-y-8 max-w-6xl pb-16 page-reveal">
          ...
          <div>
            ...
            <p className="text-white/50 mt-1 text-sm font-medium">Browse guides and find answers before raising a ticket</p>
          </div>

          {/* Filter Bar */}
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-[#111827] border border-white/10 rounded-2xl p-4">
            ...
            <div className="w-full md:w-64">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white placeholder:text-white/20 focus:outline-none focus:border-orange-500 transition-all"
                value={searchQuery}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {visible.map((article) => (
                <div
                  ...
                  className={`group bg-[#111827] border border-white/10 rounded-2xl p-6 hover:border-orange-400/40 hover:shadow-[0_0_20px_rgba(249,115,22,0.08)] transition-all cursor-pointer flex flex-col justify-between ${BORDER_COLOR_DARK[article.type]}`}
                >
                  <div>
                    ...
                    <h3 className="text-lg font-bold text-white mb-2 line-clamp-2 group-hover:text-orange-400 transition-colors">{article.title}</h3>

                    {article.tags && (
                      <div className="flex flex-wrap gap-1 mb-4">
                        {article.tags.split(",").map((tag) => (
                          <span key={tag} className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-white/5 text-white/40 border border-white/10">
                            {tag.trim()}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="border-t border-white/5 pt-4 flex justify-between items-center mt-4 text-xs text-white/50">
                    <span className="font-bold text-white/70">{article.author.name}</span>
