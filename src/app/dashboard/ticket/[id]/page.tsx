      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
           ...
           <div className="flex items-center justify-between max-w-lg mx-auto mb-10 mt-4 relative">
             <div className="absolute left-0 top-4 w-full h-1 bg-slate-200 dark:bg-slate-700 z-0" />
             ...
             <div className="relative z-10 flex flex-col items-center gap-2">
               <div className={`w-8 h-8 rounded-full border-4 flex items-center justify-center ${ticket.status === 'open' ? 'border-blue-600 bg-white dark:bg-slate-800' : 'border-blue-600 bg-blue-600'}`}>
                 <div className={`w-2.5 h-2.5 rounded-full ${ticket.status === 'open' ? 'bg-blue-600' : 'bg-white'}`} />
               </div>
               <span className={`text-xs font-bold ${ticket.status === 'open' ? 'text-blue-600' : 'text-slate-900 dark:text-white'}`}>Created</span>
             </div>
             
             <div className="relative z-10 flex flex-col items-center gap-2">
               <div className={`w-8 h-8 rounded-full border-4 flex items-center justify-center ${ticket.status === 'in_progress' ? 'border-blue-600 bg-white dark:bg-slate-800' : ticket.status === 'resolved' ? 'border-blue-600 bg-blue-600' : 'border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800'}`}>
                 <div className={`w-2.5 h-2.5 rounded-full ${ticket.status === 'in_progress' ? 'bg-blue-600' : ticket.status === 'resolved' ? 'bg-white' : 'bg-slate-300 dark:bg-slate-600'}`} />
               </div>
               <span className={`text-xs font-bold ${ticket.status === 'in_progress' ? 'text-blue-600' : ticket.status === 'resolved' ? 'text-slate-900 dark:text-white' : 'text-slate-400'}`}>In Progress</span>
             </div>
             
             <div className="relative z-10 flex flex-col items-center gap-2">
               <div className={`w-8 h-8 rounded-full border-4 flex items-center justify-center ${ticket.status === 'resolved' ? 'border-green-500 bg-white dark:bg-slate-800' : 'border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800'}`}>
                 <div className={`w-2.5 h-2.5 rounded-full ${ticket.status === 'resolved' ? 'bg-green-50' : 'bg-slate-300 dark:bg-slate-600'}`} />
               </div>
           ...
           <div className="card dark:bg-slate-800 dark:border-slate-700 p-6">
              <div className="flex items-start gap-4 mb-6 border-b border-slate-100 dark:border-slate-700 pb-6">
                 <div className="w-12 h-12 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-2xl flex-shrink-0">
                    {ticket.type === 'IT' ? '💻' : '📋'}
                 </div>
                 <div>
              ...
              {ticket.type === 'IT' && (ticket.category || ticket.softwareName || ticket.affectedSystem || ticket.errorMessage) && (
                <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700 space-y-4">
                  <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Software Details</h3>
                  <div className="grid grid-cols-2 gap-4">
                    {ticket.category && (
                      <div>
                        <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Type</p>
                        <span className="text-xs font-bold px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-blue-900 text-blue-700 dark:text-blue-300">{ticket.category}</span>
                      </div>
                    )}
                  ...
                  {ticket.errorMessage && (
                    <div>
                      ...
                      <pre className="text-xs text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-3 whitespace-pre-wrap font-mono overflow-x-auto">{ticket.errorMessage}</pre>
                    </div>
                  )}
                </div>
              )}
           ...
           {ticket.status === 'resolved' && (
             <div className="card p-6 bg-green-50/50 dark:bg-green-900/20 border-green-100 dark:border-green-800">
                <div className="flex items-center gap-2 mb-4">
                   <span className="text-green-600 text-lg">✅</span>
                   <h3 className="text-base font-bold text-green-900 dark:text-green-300">Resolution Details</h3>
                </div>
                <div className="bg-white dark:bg-slate-900 rounded-lg p-4 border border-green-100 dark:border-green-800 text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
                   {ticket.solution}
                </div>
                <div className="mt-4 flex items-center gap-2 text-xs text-green-700 dark:text-green-400">
           ...
           {isStaff && ticket.status !== 'resolved' && (
             <div className="card dark:bg-slate-800 dark:border-slate-700 p-6 border-blue-100">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Submit Resolution</h3>
                <form onSubmit={handleResolve} className="space-y-4">
                   <textarea
                     ...
                     className="input-field dark:bg-slate-900 dark:border-slate-700 dark:text-white min-h-[120px] resize-y"
                   />
               ...
               <div className="space-y-2">
                 {attachments.map(a => (
                   <a key={a.id} href={a.url} target="_blank" rel="noopener noreferrer"
                     className="flex items-center gap-3 p-3 rounded-lg border border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
           ...
           <div className="card dark:bg-slate-800 dark:border-slate-700 p-6 border-slate-200">
                 ...
                 <div className="space-y-4">
                   <div className="flex gap-4 animate-pulse">
                     ...
                     <div className="space-y-2 flex-1">
                       ...
                       <div className="h-16 bg-slate-200 dark:bg-slate-700 rounded-lg w-3/4" />
                     </div>
                   </div>
                          ...
                          <div key={item.id} className={`flex gap-3 ${isMe ? "flex-row-reverse" : ""}`}>
                            ...
                            <div className={`flex flex-col ${isMe ? "items-end" : "items-start"} max-w-[80%]`}>
                              <div className={`flex items-baseline gap-2 mb-1 ${isMe ? "flex-row-reverse" : ""}`}>
                                ...
                                {isInternal && <span className="text-xs text-amber-600 dark:text-amber-400 font-bold">🔒 Staff Only</span>}
                              </div>
                              <p className={`text-sm p-3 rounded-2xl whitespace-pre-wrap ${
                                isInternal ? "bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800" :
                                isMe ? "bg-blue-600 text-white rounded-tr-sm" : "bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 border border-slate-100 dark:border-slate-700 rounded-tl-sm"
                              }`}>
                                {item.content}
                              </p>
                            </div>
                        ...
                        return (
                          <div key={item.id} className="flex gap-4 items-center text-sm text-slate-500 dark:text-slate-400">
                            ...
                            <div className="flex-1">
                              <span className="font-bold text-slate-700 dark:text-white">{item.user?.name}</span>{" "}
             ...
             <form onSubmit={handlePostComment} className="flex gap-3 items-start border-t border-slate-100 dark:border-slate-700 pt-6 sticky bottom-0 bg-white dark:bg-slate-800 z-10 pb-2">
               <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 flex items-center justify-center font-bold flex-shrink-0">
               ...
               <div className="flex-1 space-y-3">
                 <textarea
                   ...
                   className={`input-field dark:bg-slate-900 dark:border-slate-700 dark:text-white min-h-[80px] text-sm resize-y ${isInternal ? "bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800" : ""}`}
                   required
                 />
            ...
            <div className="card p-5 space-y-6 dark:bg-slate-800 dark:border-slate-700">
               ...
               <div className="space-y-4">
                 ...
                 <div className="pt-4 border-t border-slate-100 dark:border-slate-700">
                    ...
                    {ticket.assignee ? (
                       <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-blue-50 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold">
                       ...
                       <span className="badge badge-slate dark:bg-slate-700 dark:text-slate-300">Unassigned</span>
                    )}
                 </div>
