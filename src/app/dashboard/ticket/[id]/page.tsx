        ...
        <div className="lg:col-span-2 space-y-6">
           ...
           <div className="flex items-center justify-between max-w-lg mx-auto mb-10 mt-4 relative">
             ...
             <div className="relative z-10 flex flex-col items-center gap-2">
               ...
               <span className={`text-xs font-bold ${ticket.status === 'in_progress' ? 'text-blue-600' : ticket.status === 'resolved' ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-500'}`}>In Progress</span>
             </div>
             ...
             <div className="relative z-10 flex flex-col items-center gap-2">
               ...
               <span className={`text-xs font-bold ${ticket.status === 'resolved' ? 'text-green-600' : 'text-slate-500 dark:text-slate-500'}`}>Resolved</span>
             </div>
           </div>
              ...
              {ticket.type === 'IT' && (ticket.category || ticket.softwareName || ticket.affectedSystem || ticket.errorMessage) && (
                <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800  space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {ticket.category && (
                      <div>
                        <p className="text-xs font-bold text-slate-500 dark:text-slate-500  uppercase tracking-wider mb-1">Type</p>
                    ...
                    {ticket.affectedSystem && (
                      <div>
                        <p className="text-xs font-bold text-slate-500 dark:text-slate-500  uppercase tracking-wider mb-1">Platform</p>
                        <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 ">{ticket.affectedSystem}</p>
                      </div>
                    )}
                    {ticket.softwareName && (
                      <div className="col-span-2">
                        <p className="text-xs font-bold text-slate-500 dark:text-slate-500  uppercase tracking-wider mb-1">Affected Software</p>
                  ...
                  {ticket.errorMessage && (
                    <div>
                      <p className="text-xs font-bold text-slate-500 dark:text-slate-500  uppercase tracking-wider mb-2">Error Message</p>
           ...
           <div className="card   p-6">
             ...
             {attachments.length === 0 ? (
               <p className="text-sm text-slate-500 dark:text-slate-500  italic">No attachments yet.</p>
             ) : (
                          ...
                          <div key={item.id} className={`flex gap-3 ${isMe ? "flex-row-reverse" : ""}`}>
                            ...
                            <div className={`flex flex-col ${isMe ? "items-end" : "items-start"} max-w-[80%]`}>
                              ...
                              <p className={`text-sm p-3 rounded-2xl whitespace-pre-wrap ${
                                ...
                                isMe ? "bg-blue-600 text-slate-900 dark:text-white rounded-tr-sm" : "bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300 border border-slate-100 dark:border-slate-800 rounded-tl-sm"
                              }`}>
                                {item.content}
                              </p>
                            </div>
                 ...
                 {ticket.slaResolutionDue && ticket.status !== "resolved" && (
                   <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                     <div className="flex items-center gap-1 mb-2">
                       ...
                       <span className="text-xs text-slate-500 dark:text-slate-500 cursor-help" title="SLA = Service Level Agreement. This shows whether the ticket was handled within the agreed response time.">?</span>
                     </div>
                 ...
                 {ticket.status === "resolved" && (
                  ...
                  {isCreator && (ticket.status === 'resolved' || ticket.status === 'closed') && (
                    <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-500 text-center mt-2">Issue not resolved? Reopen to submit again.</p>
                    </div>
                  )}
