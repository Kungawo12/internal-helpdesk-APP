  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/admin-portal/setup", {
        ...
        body: JSON.stringify({ name, email, password, companyPasskey }),
      });
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-8 backdrop-blur-xl shadow-[0_8px_32px_0_rgba(0,0,0,0.4)]">
          ...
          {state === "available" && (
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-xs font-bold text-white/60 uppercase tracking-widest mb-2">Company Passkey</label>
                <div className="relative">
                  <input
                    ...
                    placeholder="Set by your company (ADMIN_PASSKEY)"
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 pr-12 text-white placeholder:text-white/30 focus:outline-none focus:border-red-500/50 focus:ring-1 focus:ring-red-500/30 transition-all font-mono"
                  />
                  <button
                    type="button"
              ...
              <button
                type="submit"
                disabled={loading || !companyPasskey || !name || !email || !password}
