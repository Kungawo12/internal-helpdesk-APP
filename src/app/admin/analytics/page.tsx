  ...
  return (
    <div className="space-y-8 max-w-6xl pb-16 page-reveal">
      ...
      <div>
        ...
        <p className="text-white/30 mt-1 text-sm font-medium">Platform performance and metrics</p>
      </div>

      {/* Section 1 - KPI Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/8 transition-all">
          <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-1">SLA Compliance</p>
          <p className={`text-3xl font-extrabold ${complianceColor}`}>{data.sla.complianceRate}%</p>
          <p className="text-xs text-white/30 mt-1">target: 80%</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/8 transition-all">
          <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-1">SLA Breached</p>
          <p className={`text-3xl font-extrabold ${data.sla.totalBreached > 0 ? "text-red-400" : "text-white"}`}>
            {data.sla.totalBreached}
          </p>
          <p className="text-xs text-white/30 mt-1">active tickets past deadline</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/8 transition-all">
          <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-1">MTTR (Overall)</p>
          <p className="text-3xl font-extrabold text-white">{data.mttr.all}h</p>
          <p className="text-xs text-white/30 mt-1">mean time to resolution</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/8 transition-all">
          <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-1">Resolved Tickets</p>
          <p className="text-3xl font-extrabold text-emerald-400">
            {data.byDepartment.IT.resolved + data.byDepartment.HR.resolved}
          </p>
          <p className="text-xs text-white/30 mt-1">total resolved across depts</p>
        </div>
      </div>

      {/* Section 2 - Weekly Volume */}
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
        <h2 className="text-xs font-bold text-white/40 uppercase tracking-widest mb-6">Weekly Ticket Volume</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          <h2 className="text-xs font-bold text-white/40 uppercase tracking-widest mb-4">MTTR by Department</h2>
            ...
            {[
              ...
              <div key={r.label}>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-white font-bold">{r.value}h</span>
                </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          <h2 className="text-xs font-bold text-white/40 uppercase tracking-widest mb-4">Open Ticket Age</h2>
          <div className="space-y-4">
            {[
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          <h2 className="text-xs font-bold text-white/40 uppercase tracking-widest mb-4">SLA by Department</h2>
          <div className="space-y-6">
            {([ "IT", "HR" ] as const).map((dept) => {
              ...
              return (
                <div key={dept} className="border-b border-white/5 pb-4 last:border-0 last:pb-0">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center text-xs">
                    ...
                    <div className="bg-white/3 p-2 rounded-lg">
                      <p className="text-white/40 mb-0.5">Breached</p>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
        <div className="p-6 border-b border-white/10 bg-white/3">
          <h2 className="text-xs font-bold text-white/40 uppercase tracking-widest">Top Resolvers</h2>
        </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-white/10 bg-white/3">
                  <th className="p-4 text-xs font-bold text-white/40 uppercase tracking-widest w-16">Rank</th>
                  ...
                  <th className="p-4 text-xs font-bold text-white/40 uppercase tracking-widest text-right">Tickets Resolved</th>
                </tr>
              </thead>
              <tbody>
                {data.topResolvers.map((r, i) => (
                  <tr key={r.name} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                    <td className="p-4 font-bold text-white/40">{i + 1}.</td>
